...
class RuntypeParser {
  ...
  visit (node) {
    ...
    switch (node.kind) {
      ...
      case SyntaxKind.Identifier:
        ...
        if (this.cache.typeReferenceOpen) {
          if (this.cache.interfaceOpen) {
            this.serializeInterfacePropType('reference', key)
          } else {
            this.serializeAliasType('reference', key)
          }
      ...
      case SyntaxKind.BooleanKeyword:
        if (this.cache.interfaceOpen) {
          this.serializeInterfacePropType('primitive', node.kind)
        } else {
          this.serializeAliasType('primitive', node.kind)
        }
        break
   ...
   * @return {undefined}
   */
  serializeInterfacePropType (type, kind) {
   ...
   * @return {undefined}
   */
  serializeInterfacePropValue (type, value) {
    const prop = this.cache.currentInterface[0].props[this.cache.currentInterfacePropName]
    if (!prop.types) prop.types = []
    if (!prop.values) prop.values = []
    prop.types.push({
