...
class RuntypeParser {
  ...
  visit (node) {
    ...
    switch (node.kind) {
      ...
      case SyntaxKind.Identifier:
        if (this.cache.importOpen) break
        const key = node.escapedText

        if (key === this.cache.lookaheadIdentifierName) {
          this.cache.lookaheadIdentifierName = null
          break
        }

        if (this.cache.interfaceOpen) this.interfaceIdentifier(node, key)
        else if (this.cache.enumOpen) this.enumIdentifier(key)
      ...
      case SyntaxKind.BooleanKeyword:
        if (this.cache.interfaceOpen) this.serializeInterfaceProp('primitive', node.kind, this.cache.arrayType)
        else this.serializeAliasValue('primitive', node.kind, this.cache.arrayType)
        this.cache.arrayType = false
        break
  ...
  interfaceIdentifier (node, key) {
    const currentInterface = this.cache.currentInterface[0]
    if (currentInterface && currentInterface.name === key) return
    ...
    if (node.parent.right) {
      this.cache.lookaheadIdentifierName = node.parent.right.escapedText
      value = key + '.' + node.parent.right.escapedText
    }

    if (this.cache.typeReferenceOpen) {
      this.serializeInterfaceProp('reference', value, this.cache.arrayType)
  ...
  enumIdentifier (key) {
    ...
    if (currentEnum && currentEnum.name === key) return
   ...
   * @return {undefined}
   */
  serializeAliasValue (type, kind, list) {
  ...
  /**
   * Takes a value for an enum and serializes it into the current enum
  ...
  /**
   * Takes in a literal value and decides to serialize it into an Interface
   * or an alias
   * @param  {mixed} value The value being serialized
   * @return {undefined}
   */
