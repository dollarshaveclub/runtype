...
class RuntypeParser {
  ...
  visit (node) {
    ...
    switch (node.kind) {
      ...
      case SyntaxKind.StringLiteral:
        if (this.cache.lastTypeNodeOpen) this.serializeInterfacePropValue(node.getText().replace(/^('|")|('|")$/g, '')) // Remove double quotes
      ...
      case SyntaxKind.FirstLiteralToken:
        ...
        if (this.cache.lastTypeNodeOpen) this.serializeInterfacePropValue(value)
        this.cache.lastTypeNodeOpen = false
        break

      // Parse false
      case SyntaxKind.FalseKeyword:
        if (this.cache.lastTypeNodeOpen) this.serializeInterfacePropValue(false)
        this.cache.lastTypeNodeOpen = false
        break

      // Parse true
      case SyntaxKind.TrueKeyword:
        if (this.cache.lastTypeNodeOpen) this.serializeInterfacePropValue(true)
        this.cache.lastTypeNodeOpen = false
        break
  ...
  serializeInterfacePropType (kind) {
    ...
    prop.types.push(SyntaxKind[kind].replace('Keyword', '').toLowerCase())
  }
   ...
   * @return {undefined}
   */
  serializeInterfacePropValue (value) {
    const prop = this.cache.currentInterface[0].props[this.cache.currentPropName]
    if (!prop.types) prop.types = []
    if (!prop.values) prop.values = []
